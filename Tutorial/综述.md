...
##开发工具
chrome和火狐插件，可以插件dom树

![Alt text](https://facebook.github.io/react/img/tutorial/devtools.png "Optional title")

他可以让你检查dom树中组件的 props  和 state。
因为多个frames，它不会在CodePen中正常运行。
你可以改变视图、在新窗口打开代码，那样就会正常运行。
...
开始这个例子: <a href="https://codepen.io/ericnakagawa/pen/vXpjwZ?editors=0010" target="_blank">Starter Code</a>.
...
现在，改变render方法，显示 this.state.value 而非this.props.value,
改变事件处理为()=>this.setState({value:'X'})，而非alert。
...
Square no longer keeps its own state; it receives its value from its parent `Board` and informs its parent when it's clicked. We call components like this **受控组件s**.
...
当你渲染一个列表的时候，react已经存储了列表中的每一条信息。
如果你渲染一个有状态的组件，这个状态需要存储，不管你如何执行你的组件
React存储对后端本地视图的引用。
...
当你更新这个列表时，react需要决定哪些改变了。
你可增加、移除、重排或者更新列表中的条目。
...
```
人眼看来，很可能Alexa和Ben交换了地方，Claudia被添加了
但是react仅是个计算机，不知道你试图做什么。
因此，React要求您为列表中的每个元素指定一个键属性，一个字符串来区分每个组件与其同级。
在这个场景下，alexa、ben、claudia可能是合理的值；
如果项目对应于数据库中的对象，则数据库ID通常是一个不错的选择：
